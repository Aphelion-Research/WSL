...
int main(int argc, char **argv) {
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--help" || arg == "-h") {
      std::cout << "ragd --config PATH --db PATH --host HOST --port PORT --path PATH [--daemon] [--once-health] [--index-once]\n";
      return 0;
    }
  }
  try {
    ...
    std::unique_ptr<ragd::Watcher> watcher;
    if (cfg.watch) {
      watcher = std::make_unique<ragd::Watcher>(cfg, indexer);
      watcher->start();
    }

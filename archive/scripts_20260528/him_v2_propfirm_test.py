"""Test Him V2 zero-cost model with PROP FIRM costs.

Prop firm covers spread/slippage, only $0.50 commission per 0.1 lot.
"""
import sys
import pandas as pd
import numpy as np
...
print("=" * 60)
print("HIM V2 ZERO-COST MODEL WITH PROP FIRM COSTS")
print("=" * 60)
print("Prop firm covers spread + slippage")
print("Commission: $0.50 per side ($1.00 round-trip per 0.1 lot)")
print()
...
# Load model
model = xgb.Booster()
model.load_model("output_him_v2/him_v2_zero_cost.json")
...
propfirm_cost = CostModel(
    spread_points=0.0,      # covered
    slippage_points=0.0,    # covered
    commission_per_lot=5.0, # $0.50 per 0.1 lot = $5/lot
    lot_size=100.0,
)

print("\n" + "=" * 60)
print("RESULTS WITH PROP FIRM COSTS")
print("=" * 60)
...
for threshold in [0.50, 0.55, 0.60, 0.65]:
    ...
    print(f"  Avg PnL/trade: ${result['metrics']['total_pnl_net']/len(result['trades']):.2f}" if len(result['trades']) > 0 else "")
...
if result['metrics']['total_pnl_net'] > 0:
    print(f"\n✅ PROFITABLE at prop firm costs")
    print(f"   Threshold: {best_threshold}")
    print(f"   Total PnL: ${result['metrics']['total_pnl_net']:.2f}")
    print(f"   Trades: {len(result['trades'])}")
    print(f"   Avg: ${result['metrics']['total_pnl_net']/len(result['trades']):.2f}/trade")
    print(f"   Sharpe: {result['metrics']['sharpe']:.2f}")
else:
    print(f"\n❌ STILL UNPROFITABLE")

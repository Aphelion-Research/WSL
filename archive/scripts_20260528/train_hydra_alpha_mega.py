...
OUTPUT_DIR = Path("./output_hydra_alpha_mega")
...
def load_data():
    console.print("\n[bold cyan]═══ LOAD DATA (ALPHA) ═══[/bold cyan]")
    dataset_path = "data/hydra_alpha_dataset.parquet"
    df = pl.read_parquet(dataset_path)
    console.print(f"  {df.shape[0]:,} rows x {df.shape[1]:,} cols (tick+leadlag+regime)")
    return df


def get_feature_cols(df):
    """All valid feature columns."""
    exclude = {"time", "timestamp", "open", "high", "low", "close", "tick_volume", "spread", "real_volume"}

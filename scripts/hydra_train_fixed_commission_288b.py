...
def run_baselines(df_pd, lot_size=1.0, progress_callback=None):
    ...
    baseline_configs = [
        ...
        ('mean_reversion_12', lambda: np.sign(pd.Series(close_prices).rolling(12).mean().fillna(method='bfill').values - close_prices)),
        ('mean_reversion_72', lambda: np.sign(pd.Series(close_prices).rolling(72).mean().fillna(method='bfill').values - close_prices)),
        ('mean_reversion_288', lambda: np.sign(pd.Series(close_prices).rolling(288).mean().fillna(method='bfill').values - close_prices)),
